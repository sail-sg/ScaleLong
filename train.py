...
def train(config):
    ...
    with open(config.workdir+'/eval.log', 'r') as file:
        step_fid_data = []
        for line in file:
            parts = line.strip().split()
            step = int(parts[0].split('=')[1])
            fid10000 = float(parts[1].split('=')[1])
            step_fid_data.append((step, fid10000))

    logging.info(f'Finish fitting, step={train_state.step}')
    logging.info(f'step_fid: {step_fid_data}')
    step_best = sorted(step_fid_data, key=lambda x: x[1])[0][0]
